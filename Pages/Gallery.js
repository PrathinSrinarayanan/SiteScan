...
import { Search, MapPin, Calendar, Loader2, Image as ImageIcon, Hash } from 'lucide-react';
...
export default function Gallery() {
    ...
    const filteredArtifacts = artifacts.filter(artifact => {
        ...
        return (
            artifact.id_number?.toLowerCase().includes(query) ||
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        {filteredArtifacts.map((artifact) => {
                            ...
                            <Link key={artifact.id} to={createPageUrl('ArtifactView') + `?id=${artifact.id}`}>
                                ...
                                >
                                <div className="relative h-64 overflow-hidden" style={{ backgroundColor: `${artifactColor}15` }}>
                                    <img
                                ...
                                <div className="p-5">
                                    {artifact.id_number && (
                                        <div className="flex items-center gap-1 mb-2">
                                            <Hash className="w-3 h-3 text-[#2D5F4C]" />
                                            <span className="text-xs font-mono text-[#2D5F4C]">{artifact.id_number}</span>
                                        </div>
                                    )}
